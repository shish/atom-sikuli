...
import { parse_pattern, write_pattern } from './pattern';
/** @jsx etch.dom */

function bufs_to_pattern(bufs) {
  let pattern = {
    name: bufs.name.getText(),
    similarity: bufs.similarity.getText(),
    offset: bufs.offset.getText(),
  };
}
...
export default class ImagePropsView {
  ...
  _test(event) {
    let new_text = write_pattern(bufs_to_pattern(this.buffers));
